      ...
      <div className="bg-white shadow-2xl rounded-3xl p-10 max-w-4xl w-full text-center space-y-8">
        ...
        <div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            <FeatureCard icon={<FileText className="w-6 h-6 text-blue-600" />} label="Analisis CV" />
            <FeatureCard icon={<ClipboardList className="w-6 h-6 text-green-600" />} label="Analisis Kontrak Kerja" />
            <FeatureCard icon={<MailOpen className="w-6 h-6 text-purple-600" />} label="Generator Surat Lamaran" />
            <FeatureCard icon={<MessageCircle className="w-6 h-6 text-pink-600" />} label="Konsultasi dengan Asisten AI" />
          </div>
        </div>
      </div>
    </div>
...
function FeatureCard({ icon, label }) {
  return (
    <button className="group border border-gray-300 rounded-2xl px-6 py-5 bg-white hover:shadow-xl transition-all hover:-translate-y-1">
